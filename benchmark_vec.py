"""
Benchmark: the engine's array path against the scalar loop on a full-chain workload.
...
Both paths call the same functions: arrays take the vectorized branch, scalars the
reference branch.
...
def main():
    ...
    t_price_vec = best_of(lambda: bs_price(SPOT, K, T, R, SIGMA, OT, Q), reps=5)
    ...
    t_vec = best_of(lambda: bs_greeks(SPOT, K, T, R, SIGMA, OT, Q), reps=5)
    ...
    prices = bs_price(SPOT, K, T, R, SIGMA, OT, Q)
    iv_vec = implied_vol(prices, SPOT, K, T, R, OT, Q)
